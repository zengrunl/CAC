...
parser.add_argument('--output', dest='output', type=str, default="/root/data1/LSR/LSR2/face_50frame/")
...
audio_path = '/root/data1/LSR/LSR2/lrs2_rebuild/audio/wav16k/min/tr/mix/'
videos_path = '/root/data1/LSR/LSR2/right/'
...
for video_file in file_paths:
    ...
    while retries < max_retries:
        try:
            videogen = skvideo.io.vreader(video_file)
            lastframe = next(videogen)  # 尝试获取下一帧
            break  # 如果成功，则跳出循环
        except ValueError as e:
            print(f"Error: {e}. Retrying...")
            retries += 1  # 出错则增加重试计数器
            time.sleep(1)  # 等待一秒后重试，避免立即重试可能导致同样的错误

    fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
    video_path_wo_ext, ext = os.path.splitext(video_file)

    # Split the path to get parts. Note that this will work on Linux systems.
    path_parts = video_path_wo_ext.split('/')
    file_name = path_parts[6] + '*2'+'.mp4'
...
#     videogen = videogen[1:]
    ...
    if args.png:
        ...
        if args.output is not None:
            vid_out_name = os.path.join(args.output,file_name)
        else:
