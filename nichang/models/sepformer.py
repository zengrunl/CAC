...
class Encoder(nn.Module):
    ...
    torch.Size([2, 64, 499])
    """
    def __init__(self, kernel_size=4, out_channels=256, in_channels=1):
...
class Cross_Dual_Computation_Block(nn.Module):
    ...
    def forward(self, x, v):
        ...
        v = v.permute(0, 2, 3, 1).contiguous().view(B * K, -1, N)
...
class Cross_Dual_Path_Model(nn.Module):
    ...
    def forward(self, x, video):
        ...
        # v = video
        v = v.view(2,128,50,-1)
        v = F.pad(v, (0, 41, 0, 0), mode='replicate')
        # [B, N, K, S]
...
class Cross_Sepformer(nn.Module):
    ...
    def forward(self, mix, video):
        ...
        sep_h = [mix_w * est_mask[i] for i in range(self.num_spks)]
...
def Cross_Sepformer_warpper_1(kernel_size=16, N_encoder_out=128, num_spks=1):
