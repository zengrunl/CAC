...
class CrossTransformer(nn.Module):
    ...
    def forward(self, audio, video):
        ...
        x = self.transformer_layers(audio)
        return x

class CrossTransformerBlock(nn.Module):  #inter
    def __init__(
        self,
        num_layers,
        d_model,
...
class Cross_Dual_Computation_Block(nn.Module):
    ...
    def __init__(
        ...
        self.conv = nn.Conv1d(164, 162, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, v):
        ...
        if self.skip_around_intra:
            intra = intra + x
        # inter RNN
        # x_av = torch.cat((intra, v), dim=3)
        # intra = intra.view(B * N, S, K)
        # intra = self.conv(intra)
        # [BK, S, N]
        inter = intra.permute(0, 2, 3, 1).contiguous().view(B * K, -1, N)
        # [BK, S, H]
        # B_v, N_v, S_v = v.shape
        # v = v + pe[:, :, K // 2, :]
        ...
        # [B, K, S, N]
        inter = inter.view(B, K, -1, N)
        # [B, N, K, S]
        inter = inter.permute(0, 3, 2, 1).contiguous().view(B*N, -1, K)
        # inter = self.conv(inter)
        inter = inter.view(B, N, -1, K)
...
class Cross_Dual_Path_Model(nn.Module):
    ...
    def __init__(
        ...
        self.conv = nn.Conv1d(100, 50, kernel_size=1, stride=1, padding=0)
        # gated output layer
    ...
    def forward(self, x, video):
        ...
        # [B*spks, N, K, S]
        x = x.view(B * self.num_spks, _, K,-1)
...
def Cross_Sepformer_warpper(kernel_size=16, N_encoder_out=256, num_spks=2):
