...
# class Cross_Model_Trans(nn.Module):
#     def __init__(self, d_model, nhead, dropout=0.1):
#         super(Cross_Model_Trans, self).__init__()
#
#         # 定义多头自注意力层
...
class Sandglasset_Block(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels=128, num_layers=1, bidirectional=True, num_heads=8,
        ...
        self.visual_Conv1D = nn.Conv1d(in_channels=in_channels,
                                out_channels=in_channels,
                                kernel_size=kernel_size,
                                stride=stride,
                                padding=0)
        ...
        self.cross_attention = nn.MultiheadAttention(embed_dim=128,dropout=0.1,num_heads=8)
    ...
    def forward(self, x, v):
        ...
        v = self.visual_Conv1D(v)

        x = x.view(B, K, N, -1).permute(0, 2, 1, 3).contiguous()
        x = self.Globally_Attentive(x)

        v = v.permute(0, 2, 1)
        v = self.MultiheadAttention(v, v, v, attn_mask=None, key_padding_mask=None)[0]
        v = v.permute(0, 2, 1)


        x = x.view(B * K, N, -1).permute(2,0,1).contiguous()
        residual1 = x
        v = v.permute(2,0,1)
        x,_ = self.cross_attention(x, v, v)
        x = x + residual1
        x = x.permute(1, 2, 0)
        v = v.permute(1, 2, 0)
...
class Separation(nn.Module):
    ...
    def forward(self, x, v):
        ...
        for i in range(self.cycle_amount):
            ...
            x = self.Sandglasset_Blocks[i](x, v)  # torch.Size([1, 64, 200, 322])
            padding_size = S - x.size(3)
            if padding_size > 0:
                # pad 的参数是一个四元组，格式为 (左, 右, 上, 下)，因为我们只填充最后一个维度，所以其他维度都是 0
                x = F.pad(x, (0, padding_size, 0, 0))
            if i < self.cycle_amount//2:
...
class Sandglasset(nn.Module):
    ...
    def forward(self, x, v):
        ...
        s = self.Separation(e,v)  # torch.Size([2, 1, 128, 31999])
