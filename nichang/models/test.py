...
class GatedMultimodalLayer(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        return z * h1_transformed + (1 - z) * h2_transformed,z
class GridNetBlock(nn.Module):
    ...
    def forward(self, x, complement1,complement2):
        ...
        inter_rnn = input_  + inter_rnn + x_inter+ complement1 + complement2 # [B, C, T, Q]
        ...
        return out,self.a,complement1,complement2
...
class TF_Patch(nn.Module):
    ...
    def forward(self, input, mouth):
        ...
        batch_comp = self.conv1d_(batch_o.view(n_batch, _, n_frames, n_freqs))

        mouth_ssm = mouth_ssm + mouth.view(n_batch*_, n_frames, n_freqs)
        mouth_comp = self.mouth_pool(mouth_ssm.view(n_batch, _, n_frames, n_freqs))
        ...
        batch2 = batch
        #
        # mouth_ssm = mouth_ssm.view(n_batch, _, n_frames, n_freqs)
        ...
        # 应用avgpool和maxpool
        input_c = batch_o.view(n_batch, _, n_frames, n_freqs)
        input_p = mouth_o.view(n_batch, _, n_frames, n_freqs)

        input_p = self.conv_g(input_p)
        input_p = self.mouth_norm(input_p)

        input_c = self.conv_g_1(input_c)
        input_c = self.mouth_norm(input_c)


        for ii in range(self.n_layers):
            batch,a,input_c,input_p = self.blocks[ii](batch,input_c,input_p )  # [B, -1, T, F]
        ...
        return batch,input_c, input_p,a,batch2,mouth_comp,batch_comp
