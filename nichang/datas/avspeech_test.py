...
class AVSpeechDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if self.n_src == 2:
            ...
            for i in range(2):
                sources.append(sf.read(self.sources[i][idx][0], start=rand_start, stop=stop, dtype="float32"))
                sources_mouths.append(torch.from_numpy(self.lipreading_preprocessing_func(np.load(self.sources[i][idx][1])["data"])))
            ...
            # ]
            # import pdb; pdb.set_trace()
            sources = torch.stack([torch.from_numpy(source[0]) for source in sources])
            mixture = torch.from_numpy(mix_source)
            if self.normalize_audio:
                m_std = mixture.std(-1, keepdim=True)
                mixture = normalize_tensor_wav(mixture, eps=self.EPS, std=m_std)
            ...
            return mixture[:self.sample_rate * 2], sources[:self.sample_rate * 2], \
                torch.stack(sources_mouths,dim=0), self.mix[idx][0].split("/")[-1]
