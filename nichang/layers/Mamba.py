...
class Mamba(nn.Module):
    def __init__(
        ...
        self.in_proj_y = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)
    ...
    def forward(self, hidden_states, inference_params=None, visual_audio = None):
        """
        hidden_states: (B, L, D)
        ...
        yz = rearrange(
            self.in_proj_y.weight @ rearrange(visual_audio, "b l d -> d (b l)"),
            "d (b l) -> b d l",
            l=seqlen,
        )
        if self.in_proj.bias is not None:
            xz = xz + rearrange(self.in_proj.bias.to(dtype=xz.dtype), "d -> d 1")
            # yz = yz + rearrange(self.in_proj_y.bias.to(dtype=yz.dtype), "d -> d 1")

        A = -torch.exp(self.A_log.float())  # (d_inner, d_state)
        A1 = -torch.exp(self.A_log.float())  # (d_inner, d_state)
        A2 = -torch.exp(self.A_log.float())
        ...
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None:  # Doesn't support outputting the states
            ...
            if causal_conv1d_fn is None:
                y_x = self.act_y(self.conv1d_y(y)[..., :seqlen])
                y_x = y_x * x
                x_ = self.act(self.conv1d(x)[..., :seqlen])  #conv
                x = x_ * y
            ...
            y_dbl = self.y_proj(rearrange(y, "b d l -> (b l) d"))
            dt, B, C = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1)
            dt1, B1, C1 = torch.split(y_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1)
            ...
            # B2 = rearrange(B1, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
            # C2 = rearrange(C1, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
            assert self.activation in ["silu", "swish"]
            y = selective_scan_fn(
                (x),
                dt,
            ...
            out = self.out_proj(y) + self.out_proj1(y1)
...
class ExBimamba(nn.Module):
    ...
    def forward(self, real_input, img_input):
        forward_output, backward_output = self.forward_mamba(hidden_states = real_input,inference_params=None, visual_audio = img_input)
