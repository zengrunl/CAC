...
class PlainMamba2D(nn.Module):
    def __init__(
        ...
        torch.nn.init.trunc_normal_(self.direction_Bs, std=0.02)
    ...
    def forward(self, x, hw_shape, mouth):
        batch_size, L, _ = x.shape  #2,8125,256
        H, W = hw_shape
        E = self.d_inner
        conv_state, ssm_state = None, None

        xz = self.in_proj(x)  # [B, L, 2 * E] 2,1134,1536   2,4536,512
        ...
        x_conv = x_2d.permute(0, 2, 3, 1).reshape(batch_size, L, E) #2,1138，764  2,4536,256

        x_dbl = self.x_proj(x_conv)  # (B, L, dt_rank + d_state * 2),2,1134,56
        dt, B, C = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1)
        dt = self.dt_proj(dt)

        dt = dt.permute(0, 2, 1).contiguous()  # [B, d_innter, L]2,768,1134
        B = B.permute(0, 2, 1).contiguous()  # [B, d_state, L]2,16,1134
        C = C.permute(0, 2, 1).contiguous()  # [B, d_state, L]2,16,1134

        assert self.activation in ["silu", "swish"]

        orders, inverse_orders= self.get_permute_order(hw_shape)
...
class xLSTM(nn.Module):
    ...
    def forward(self, x, hw_shape, mouth):
        ...
        xz = self.in_proj(x)  # [B, L, 2 * E] 2,1134,1536   2,4536,512
        x, z = xz.chunk(2, dim=-1)
        x_lstm = self.xlstm_stack(x)
        assert self.activation in ["silu", "swish"]

        # orders, inverse_orders, directions = self.get_permute_order(hw_shape)
        #
        # ys = [
        #     self.xlstm_stack(x[:, o, :])for o, inv_o in zip(orders, inverse_orders)
        # ]
        # y_mouth = mouth * self.act(z)
        y = x_lstm * self.act(z)
        y = y
        out = self.out_proj(y)
...
class PlainMambaLayer(nn.Module):
    def __init__(
        ...
        self.xlstm = xLSTM(**mamba_cfg)
        self.drop_path = build_dropout(dict(type='DropPath', drop_prob=drop_path_rate))

    def forward(self, x, hw_shape, mouth):
        mixed_x = self.drop_path(self.mamba(self.norm(x), hw_shape, mouth))
        mixed_x = mixed_x + x #2,9728,256
        if self.with_dwconv:
            b, l, c = mixed_x.shape
            h, w = hw_shape
