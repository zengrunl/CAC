...
def blocksparse_attention_impl(q, k, v1,v2, heads, attn_mode, local_attn_ctx=None, blocksize=32, num_verts=None, vertsize=None):
    n_ctx = q.size()[1]
    if attn_mode == 'strided':
        ...
        v1 = strided_transpose(v1, n_ctx, local_attn_ctx, blocksize)
        v2 = strided_transpose(v2, n_ctx, local_attn_ctx, blocksize)
    ...
    if attn_mode == 'strided':
        ...
        b = torch.reshape(b, [n, local_attn_ctx, bT_ctx, embd])
        b = torch.transpose(b, 1,2)
        b = torch.reshape(b, [n, t, embd])
...
class SparseAttention(nn.Module):
    ...
    def forward(self, q, k, v1,v2):
        return blocksparse_attention_impl(q, k, v1,v2, self.heads, self.attn_mode, self.local_attn_ctx, self.blocksize)
