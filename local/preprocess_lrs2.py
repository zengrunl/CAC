...
def get_mouth_path(in_mouth_dir, wav_file, out_filename, data_type):
    ...
    if out_filename == "s1":
        file_path = os.path.join(
            in_mouth_dir, "{}_{}.npz".format(wav_file[0], s1)
        )
    if out_filename == "s2":
        file_path = os.path.join(
            in_mouth_dir, "{}_{}.npz".format(wav_file[3], s2)
        )
    if out_filename == "s3":
        file_path = os.path.join(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--in_audio_dir",
        type=str,
        default="/root/data1/LSR/LSR2/lrs2_rebuild/audio/wav16k/min/",
        help="Directory path of audio including tr, cv and tt",
    )
    parser.add_argument(
        "--in_mouth_dir",
        type=str,
        default="/root/data1/LSR/LSR2/mouth_50/",
        help="Directory path of video including tr, cv and tt",
    )
    parser.add_argument(
        "--out_dir", type=str, default="/root/data1/LSR/data_50/", help="Directory path to put output files"
    )
